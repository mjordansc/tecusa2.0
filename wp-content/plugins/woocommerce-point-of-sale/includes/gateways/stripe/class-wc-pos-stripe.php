...
class WC_POS_Stripe {
	...
	public static function init() {
		...
		add_filter( 'wc_pos_app_data', [ __CLASS__, 'app_data' ], 10, 1 );
		add_filter( 'wc_pos_nonces', [ __CLASS__, 'nonces' ], 10, 1 );
		add_action( 'admin_notices', [ __CLASS__, 'show_stripe_notice' ] );
		add_filter( 'wc_pos_register_options_tabs', [ __CLASS__, 'register_options_tabs' ], 10, 1 );
		add_action( 'wc_pos_register_options_panels', [ __CLASS__, 'register_options_panels' ], 10, 2 );
		add_action( 'wc_pos_register_options_save', [ __CLASS__, 'save_register_data' ], 10, 1 );
		add_filter( 'wc_pos_register_data', [ __CLASS__, 'add_register_data' ], 10, 1 );
	}
	...
	/**
	 * Adds gateway params to the global object window.AppConfig.
	 *
	 * @param array $data
	 * @return array
	 */
	...
	/**
	 * Adds gateway nonces to the window.Nonces global variable.
	 *
	 * @param array $data
	 * @return array
	 */
	 ...
	 * @param int $post_id
	 */
	public static function save_register_data( $post_id ) {
