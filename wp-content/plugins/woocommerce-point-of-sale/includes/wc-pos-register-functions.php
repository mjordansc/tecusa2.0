...
function wc_pos_get_register( $register ) {
	...
	// Fallback to default register.
	if ( 0 === $register_object->get_id() ) {
		$register_object = new WC_POS_Register( $default_register_id );
	}

	// Fallback if the default register could not be retrieved.
	if ( 0 === $register_object->get_id() ) {
