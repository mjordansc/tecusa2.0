		...
		$grid_id = wp_insert_post(
			...
			apply_filters(
				...
				[
					...
					'post_content'  => '',
					'post_excerpt'  => '',
					'post_date'     => gmdate( WC_POS_SQL_DATETIME_FORMAT, $grid->get_date_created()->getOffsetTimestamp() ),
					'post_date_gmt' => gmdate( WC_POS_SQL_DATETIME_FORMAT, $grid->get_date_created()->getTimestamp() ),
				]
			),
			true
		);
	...
	public function update( &$grid ) {
		...
		if ( array_intersect( [ 'name', 'date_created', 'date_modified' ], array_keys( $changes ) ) ) {
			$post_data = [
				'post_title'        => $grid->get_name( 'edit' ),
				'post_excerpt'      => '',
				'post_date'         => gmdate( WC_POS_SQL_DATETIME_FORMAT, $grid->get_date_created( 'edit' )->getOffsetTimestamp() ),
				'post_date_gmt'     => gmdate( WC_POS_SQL_DATETIME_FORMAT, $grid->get_date_created( 'edit' )->getTimestamp() ),
				'post_modified'     => isset( $changes['date_modified'] ) ? gmdate( WC_POS_SQL_DATETIME_FORMAT, $grid->get_date_modified( 'edit' )->getOffsetTimestamp() ) : current_time( 'mysql' ),
				'post_modified_gmt' => isset( $changes['date_modified'] ) ? gmdate( WC_POS_SQL_DATETIME_FORMAT, $grid->get_date_modified( 'edit' )->getTimestamp() ) : current_time( 'mysql', 1 ),
			];
