...
class WC_POS_Admin {
	...
	public function __construct() {
		...
		if ( 'yes' === get_option( 'wc_pos_manage_outlet_stock' ) ) {
			...
			add_filter( 'woocommerce_prevent_adjust_line_item_product_stock', [ $this, 'prevent_adjust_line_item_product_stock' ], 999, 2 );
		}
					...
					<tr>
						...
						<td>
							...
							if ( isset( $field['type'] ) && 'select' === $field['type'] ) {
								...
								$value_user_meta = array_map( 'intval', $value_user_meta );
								...
								<select name="<?php echo isset( $field['name'] ) ? esc_attr( $field['name'] ) : esc_attr( $key ); ?>" id="<?php echo esc_attr( $key ); ?>" <?php echo esc_attr( $multiple ); ?> style="min-width:350px;" class="wc-enhanced-select">
									...
									foreach ( $field['options'] as $label_value => $label ) {
										echo '<option value="' . esc_attr( $label_value ) . '" ' . ( ( in_array( $label_value, $value_user_meta, true ) ) ? 'selected' : '' ) . ' >' . esc_html( $label ) . '</option>';
									}
									?>
								</select>
