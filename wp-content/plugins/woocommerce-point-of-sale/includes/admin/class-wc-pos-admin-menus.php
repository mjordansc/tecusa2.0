...
class WC_POS_Admin_Menus {
	...
	public function __construct() {
		...
		add_filter( 'submenu_file', [ $this, 'submenu_file' ], 10, 1 );
	}
	...
	public function add_menu() {
		...
		$manage_stock_roles = get_option( 'wc_pos_manage_stock_roles', [ 'outlet_manager', 'shop_manager' ] );
		$current_user       = wp_get_current_user();
		if ( in_array( 'administrator', $current_user->roles, true ) || ! empty( array_intersect( $manage_stock_roles, $current_user->roles ) ) ) {
			add_submenu_page(
				WC_POS()->menu_slug,
