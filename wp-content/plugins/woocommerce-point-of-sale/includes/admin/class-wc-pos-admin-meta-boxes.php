...
class WC_POS_Admin_Meta_Boxes {
	...
	public function __construct() {
		...
		if ( 'yes' === get_option( 'wc_pos_decimal_quantities', 'no' ) ) {
			...
			add_action( 'woocommerce_process_product_meta', [ $this, 'save_point_of_sale_options' ], 10, 1 );
		}
		...
		if ( 'yes' === get_option( 'wc_pos_manage_outlet_stock' ) ) {
			...
			add_action( 'woocommerce_process_product_meta', [ $this, 'save_outlet_stock_options' ], 999, 1 );
	...
	public function save_product_grids( $post_id, $post ) {
		...
		foreach ( $posted_grids as $grid_id ) {
			if ( ! in_array( $grid_id, $current_grids, true ) ) {
				$to_add[] = $grid_id;
			}
		}

		foreach ( $current_grids as $grid_id ) {
			if ( ! in_array( $grid_id, $posted_grids, true ) ) {
				$to_remove[] = $grid_id;
			}
		}
	 ...
	 * @param int $post_id Post id.
	 */
	public function save_outlet_stock_options( $post_id ) {
		if ( empty( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_REQUEST['_wpnonce'] ) ), 'update-post_' . $post_id ) ) {
			wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'woocommerce-point-of-sale' ) );
		}
	 ...
	 * @param int $post_id Post id.
	 */
	public function save_point_of_sale_options( $post_id ) {
		if ( empty( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_REQUEST['_wpnonce'] ) ), 'update-post_' . $post_id ) ) {
			wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'woocommerce-point-of-sale' ) );
		}
	 ...
	 */
	public function actions_panel( $post ) {
		$post = $post; // Bypass PHPCS warning.
