...
class WC_POS_Admin_List_Table_Registers extends WC_Admin_List_Table {
	...
	protected function render_status_column() {
		if ( wc_pos_is_register_open( $this->object->get_id() ) ) {
			echo wp_kses_post( $this->get_row_icon( 'open', __( 'Open', 'woocommerce-point-of-sale' ) ) );
		} else {
			echo wp_kses_post( $this->get_row_icon( 'closed', __( 'Closed', 'woocommerce-point-of-sale' ) ) );
		}
	}
	...
	protected function render_change_user_column() {
		if ( $this->object->get_change_user() ) {
			echo wp_kses_post( $this->get_row_icon( 'yes', __( 'User changes after sale.', 'woocommerce-point-of-sale' ) ) );
		} else {
			echo wp_kses_post( $this->get_row_icon( 'no', __( 'User does not change after sale.', 'woocommerce-point-of-sale' ) ) );
		}
	}
	...
	protected function render_email_receipt_column() {
		switch ( $this->object->get_email_receipt() ) {
			case 'all':
				echo wp_kses_post( $this->get_row_icon( 'yes', __( 'Receipt is emailed to all customers.', 'woocommerce-point-of-sale' ) ) );
				break;
			case 'non_guest':
				echo wp_kses_post( $this->get_row_icon( 'yes', __( 'Receipt is emailed to non-guest customers only.', 'woocommerce-point-of-sale' ) ) );
				break;
			default:
				echo wp_kses_post( $this->get_row_icon( 'no', __( 'Receipt is not emailed.', 'woocommerce-point-of-sale' ) ) );
				break;
		}
	}
	...
	protected function render_print_receipt_column() {
		if ( $this->object->get_print_receipt() ) {
			echo wp_kses_post( $this->get_row_icon( 'yes', __( 'Receipt is printed.', 'woocommerce-point-of-sale' ) ) );
		} else {
			echo wp_kses_post( $this->get_row_icon( 'no', __( 'Receipt is not printed.', 'woocommerce-point-of-sale' ) ) );
		}
	}
	...
	protected function render_note_request_column() {
		switch ( $this->object->get_note_request() ) {
			case 'on_save':
				echo wp_kses_post( $this->get_row_icon( 'yes', __( 'Note is taken on save.', 'woocommerce-point-of-sale' ) ) );
				break;
			case 'on_all_sales':
				echo wp_kses_post( $this->get_row_icon( 'yes', __( 'Note is taken on all sales.', 'woocommerce-point-of-sale' ) ) );
				break;
			default:
				echo wp_kses_post( $this->get_row_icon( 'no', __( 'Note is not taken.', 'woocommerce-point-of-sale' ) ) );
				break;
		}
	}
	...
	protected function render_cash_management_column() {
		if ( $this->object->get_cash_management() ) {
			echo wp_kses_post( $this->get_row_icon( 'yes', __( 'Cash is managed.', 'woocommerce-point-of-sale' ) ) );
		} else {
			echo wp_kses_post( $this->get_row_icon( 'no', __( 'Cash is not managed.', 'woocommerce-point-of-sale' ) ) );
		}
	}
	...
	protected function render_access_column() {
		...
		if ( class_exists( 'SitePress' ) ) {
			...
			if ( 1 === $settings['urls']['directory_for_default_language'] && defined( 'ICL_LANGUAGE_CODE' ) ) {
				$register_link = get_home_url() . '/' . ICL_LANGUAGE_CODE . "/point-of-sale/{$outlet}/{$this->object->get_slug()}";
			}
		}
	 ...
	 * @return string
	 */
	private function get_row_icon( $icon = 'yes', $tip = '' ) {
