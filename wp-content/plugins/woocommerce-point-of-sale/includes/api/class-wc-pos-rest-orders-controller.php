...
class WC_POS_REST_Orders_Controller extends WC_REST_Orders_Controller {
	...
	protected function prepare_links( $object_data, $request ) {
		return []; // Remove links.
	}
	 ...
	 */
	public function prepare_object_for_response( $object_data, $request ) {
		$response = parent::prepare_object_for_response( $object_data, $request );
		$data     = $response->get_data();

		// Include register and outlet data.
		$register = wc_pos_get_register( absint( $object_data->get_meta( 'wc_pos_register_id', true ) ) );
		if ( $register ) {
			$data['register_data'] = [
				'id'   => $register->get_id(),
		...
		// Include order note.
		$order_note_id      = $object_data->get_meta( '_wc_pos_order_note_id', true );
		$order_note         = wc_get_order_note( $order_note_id );
		$data['order_note'] = $order_note ? $order_note->content : '';

		// Include clerk details.
		$clerk_id   = (int) $object_data->get_meta( 'wc_pos_served_by', true );
		$clerk_name = $object_data->get_meta( 'wc_pos_served_by_name', true );
