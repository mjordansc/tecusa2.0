...
class WC_POS_Autoloader {
	...
	public function autoload( $class_name ) {
		...
		if ( 0 === strpos( $class_name, 'wc_pos_screen' ) ) {
			$path = $this->include_path . 'screen/';
		} elseif ( 0 === strpos( $class_name, 'wc_pos_table' ) ) {
			$path = $this->include_path . 'tables/';
		} elseif ( 0 === strpos( $class_name, 'wc_pos_meta_box' ) ) {
			$path = $this->include_path . 'admin/meta-boxes/';
		} elseif ( 0 === strpos( $class_name, 'wc_pos_admin' ) ) {
			$path = $this->include_path . 'admin/';
		}

		if ( empty( $path ) || ( ! $this->load_file( $path . $file ) && 0 === strpos( $class_name, 'wc_pos_' ) ) ) {
