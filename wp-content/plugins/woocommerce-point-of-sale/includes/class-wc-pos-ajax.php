...
class WC_POS_AJAX {
	...
	public static function check_updates() {
		...
		if ( $last_synced && ! empty( $collections ) ) {
			...
			$last_synced_datetime = gmdate( WC_POS_SQL_DATETIME_FORMAT, $last_synced );
	...
	public static function open_register() {
		...
		$date_opened = gmdate( WC_POS_ISO_DATETIME_FORMAT );
		...
		$session_data = WC_POS_App::instance()->get_session_data( $session->get_id(), false );
	...
	public static function close_register() {
		...
		$date_closed    = gmdate( WC_POS_ISO_DATETIME_FORMAT );
		...
		$session_data = WC_POS_App::instance()->get_session_data( $session_id, false );
		wc_pos_send_json_success( $session_data );
	}
	...
	public static function insert_cash_flow() {
		...
		wc_pos_send_json_success( WC_POS_App::instance()->get_session_data( $session->get_id(), false ) );
	}
	...
	public static function auth_user() {
		...
		wc_pos_send_json_success(
			[
				...
				'session_data' => WC_POS_App::instance()->get_session_data( $register->get_current_session(), false ),
			]
		);
	}
	...
	public static function session_data() {
		...
		wc_pos_send_json_success(
			[
				'session_data'        => WC_POS_App::instance()->get_session_data( $register->get_current_session(), true ),
				'logged_in_user_only' => 'yes' === get_option( 'wc_pos_display_total_sales_for_logged_in_user', 'no' ),
			]
		);
	}
	...
	public static function add_grid_tile() {
		...
		try {
			...
			if ( ! isset( $_POST['data']['tile_type'] ) || ! in_array( $_POST['data']['tile_type'], [ 'product', 'product_cat' ], true ) ) {
